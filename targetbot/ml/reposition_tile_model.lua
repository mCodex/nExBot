RepositionTileModel = {}
RepositionTileModel.__index = RepositionTileModel

local MAX_WEIGHT = 10

function RepositionTileModel.new(options)
  options = options or {}
  return setmetatable({
    _weights = {},
    _sampleCount = 0,
...
function RepositionTileModel:predict(tileFeatures)
  ...
  local z = 0
  for key, value in pairs(tileFeatures) do
...
function RepositionTileModel:observe(success, tileFeatures)
  self._sampleCount = self._sampleCount + 1
  local pred = self:predict(tileFeatures).probability
...
function RepositionTileModel:getSampleCount()
  return self._sampleCount
end

function RepositionTileModel:reset()
  self._weights = {}
  self._sampleCount = 0
end

return RepositionTileModel
