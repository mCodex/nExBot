...
describe("BotShell", function()
  ...
  it("keeps browser history for explicit navigation", function()
  ...
  it("renders persistent category and contextual tab controls", function()
    ...
    assert.is_truthy(shell:getWorkspace():recursiveGetChildById("shellBack"))
  end)

  it("uses a compact page selector when the available viewport is narrow", function()
    local root = _G.g_ui.createWidget("Root", nil)
    root:setWidth(420)
    root:setHeight(640)
    local shell = Shell.new({ root = root })
    shell:open()
    shell:select("targetbot")

    assert.is_truthy(shell:getWorkspace():recursiveGetChildById("pageSelect"))
    assert.is_nil(shell:getWorkspace():recursiveGetChildById("tab_targetbot"))
    assert.are_equal(404, shell:getWorkspace():getWidth())
  ...
  it("propagates density changes to the shared component library", function()
    local Components = _G.nExBot.UI["ui.components.components"]
    local shell = Shell.new({ root = _G.g_ui.createWidget("Root", nil) })
    assert.are_equal("default", Components.getDensity())

    assert.is_true(shell:setDensity("touch"))
    assert.are_equal("touch", shell:density())
    assert.are_equal("touch", Components.getDensity())
  end)

  it("rejects an unknown density", function()
    local shell = Shell.new({ root = _G.g_ui.createWidget("Root", nil) })
    assert.is_false(shell:setDensity("ultra"))
    assert.are_equal("default", shell:density())
  end)
